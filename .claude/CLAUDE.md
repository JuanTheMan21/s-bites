...
## Codebase Intelligence for s_bites (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-06 (commit 167299e). Confidence: 100%.
...
### Architecture
**Files:** 55 | **Lines:** 4096
s_bites is a python codebase of 55 files. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on. ---
*Built from the code's structure. It states what is there, not why it is that
way.

### Key modules
- `interfaces` — interfaces
**Language:** python | **Files:** 8 | **Public symbols:** 37 / 37
Covers the 8 source files in interfaces
- `scripts` — scripts
**Language:** python | **Files:** 5 | **Public symbols:** 22 / 22
Covers the 5 source files in scripts
- `adapters` — adapters · adapters/azure · adapters/local
**Language:** python | **Files:** 3 | **Public symbols:** 0 / 0
Covers the 3 source files in 3…
- `core` — core · core/graph · core/graph/nodes
**Language:** python | **Files:** 7 | **Public symbols:** 24 / 24
Covers the 7 source files in 3…
- `api` — api
**Language:** python | **Files:** 1 | **Public symbols:** 0 / 0
Covers the 1 source files in api
- `mux` — mux
**Language:** python | **Files:** 1 | **Public symbols:** 0 / 0
Covers the 1 source files in mux
- `rendering` — rendering
**Language:** python | **Files:** 1 | **Public symbols:** 0 / 0
Covers the 1 source files in rendering
...
- `rendering/block_timing.py` — 4 bug fixes, last fix yesterday (bug magnet); 6 commits/90d
- `tests/test_item_anchor_resolution.py` — 3 bug fixes, last fix yesterday; 4 commits/90d
- `rendering/annotations.py` — 3 bug fixes, last fix yesterday; 7 commits/90d
- `tests/graph_pipeline_fixtures.py` — 3 bug fixes, last fix yesterday; 8 commits/90d
- `rendering/compose.py` — 2 bug fixes, last fix yesterday; 7 commits/90d

### Code health
Three co-equal signals: defect risk 7.36/10 avg, hotspot health 6.77/10 (stable), worst `.claude/skills/impeccable/scripts/context.mjs` at 3.35/10 · maintainability 7.64/10 · performance risk 212 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `rendering/templates/_tokens.html` — change entropy — impact −3.0
- `rendering/compose.py` — untested hotspot — impact −2.0
- `adapters/local/render_backend.py` — untested hotspot — impact −2.0
- `core/block_types.py` — untested hotspot — impact −2.0
- `scripts/measure_segment_concurrency.py` — change entropy — impact −1.8
